...
inline void AddRow(mat3 *a, int i, int j, float32 scale) {
...
// rowI -> rowI + scale * rowJ
inline void AddRow(mat4 *a, int i, int j, float32 scale) {
    a->data[0 + i] += scale * a->data[0 + j];
...
inline void MultiplyRow(mat3 *a, int i, float32 scale) {
    a->data[0 + i] *= scale;
    a->data[3 + i] *= scale;
    a->data[6 + i] *= scale;
}

inline void MultiplyRow(mat4 *a, int i, float32 scale) {
    a->data[0 + i] *= scale;
    a->data[4 + i] *= scale;
    a->data[8 + i] *= scale;
    a->data[12 + i] *= scale;
...
inline bool Inverse(mat3 a, mat3 *inverted) {
    ...
    for (int col = 0; col < 3; col++) {
        int maxRow = col;
        float32 maxRowValue = fabsf(tmp.data[3 * col + col]);
        
        for (int row = col + 1; row < 3; row++) {
            float32 rowValue = fabsf(tmp.data[3 * col + row]);
...
inline bool Inverse(mat4 a, mat4 *inverted) {
    ...
    for (int col = 0; col < 4; col++) {
        int maxRow = col;
        float32 maxRowValue = fabsf(tmp.data[4 * col + col]);
        
        for (int row = col + 1; row < 4; row++) {
            float32 rowValue = fabsf(tmp.data[4 * col + row]);
...
inline mat4 OrthogonalInverse(mat4 a) {
    ...
    float32 invDet = 1.0f / Dot(Cross(X, Y), Z);
...
inline mat4 Scale4(float32 scale) {
...
inline mat3 Scale3(float32 scale) {
...
inline mat4 TRS(vec3 translation, mat4 rotation, float32 scale) {
...
inline mat4 TRS(vec3 translation, quaternion rotation, float32 scale) {
...
inline mat4 TS(vec3 translation, float32 scale) {
...
inline mat4 Orthographic(float32 left, float32 right, float32 bottom, float32 top, float32 nearPlane, float32 farPlane) {
    mat4 result;
...
inline mat4 Perspective(float32 vFOV, float32 aspect, float32 nearPlane, float32 farPlane) {

    float32 top = nearPlane * tanf(vFOV * 0.5f);
    float32 bottom = -top;

    float32 left = -aspect * top;
    float32 right = -left;
...
inline mat4 PerspectiveInfiniteFarPlane(float32 vFOV, float32 aspect, float32 nearPlane) {
    float32 top = nearPlane * tanf(vFOV * 0.5f);
    float32 bottom = -top;
