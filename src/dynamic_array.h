...
void DynamicArrayAllocateChunk(DynamicArray_Untyped *array, uint32 elementSize) {
  ArrayChunk *newChunk = (ArrayChunk *)AllocateMem(array->allocator, sizeof(ArrayChunk));
  newChunk->data = (uint8 *)AllocateMem(array->allocator, array->elementsPerChunk * elementSize);
...
void DynamicArrayAllocateChunk(DynamicArray<T> *array) {
  ArrayChunk *newChunk = (ArrayChunk *)AllocateMem(array->allocator, sizeof(ArrayChunk));
  newChunk->data = (uint8 *)AllocateMem(array->allocator, array->elementsPerChunk * sizeof(T));
